...
<div class="container">
	...
	<c:choose>
		<c:when test="${not empty user_id }">
			...
			<!--  
			시큐리티 설정에 logout-url="/security2/logout" 이와같이 설정하였으므로
		...
		<c:otherwise>
			...
			동일한 값(현재는 /myLoginAction)으로 action속성을 설정해야 한다.
			설정하지 않았다면 default값이 "/login"이므로 아래와 같이 기술한다.
			...
			<form:form name="loginFrm" action="../myLoginAction" method="post">
				...
				이와 같이 기술하였으므로 로그인에 실패한 경우에는 아래와 같이 
				param.error != null로 처리하면 된다.
				-->
				...
				password-parameter="pass" 이와 같이 설정하였으므로
				name속성은 꼭 아래와 같이 동일하게 기술한다.
				-->
				<p>
