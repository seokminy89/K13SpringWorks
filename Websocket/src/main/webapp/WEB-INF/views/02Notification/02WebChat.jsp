...
//채팅에서 사용할 전역변수 생성
var messageWindow; //대화 내용이 출력될 엘리먼트
...
window.onload = function(){
    ...
    //webSocket = new WebSocket('ws://192.168.219.101:8081/websocket/EchoServer.do');//학원아이피
...
function wsMessage(event){
	/*
	메세지를 |(파이프) 기호로 분리하여 앞부분은 보낸사람, 
	뒷부분은 메세지로 각각 변수에 저장한다.
	*/
    ...
    if(content == ""){
        ...
        if(content.match("/")){
        	...
        	판단한다. 아래 chat_id는 현재 페이지로드시 접속한 아이디를 통해
        	전역변수에 저장되어 있는 값이다.
        	따라서 받는 사람이 본인인 경우에만 대화내용을 디스플레이 한다.
        	*/
...
function sendMessage(){
    ...
    messageWindow.innerHTML += msg;
    inputMessage.value = "";//대화 입력창을 비운다.
    messageWindow.scrollTop = messageWindow.scrollHeight;//스크롤바를 아래로 내린다.
}
//대화 내용을 입력한 후 Enter키를 통해 메세지를 전송할 수 있다.
function enterkey(){
	//키보드를 눌렀다가 땠을때 키코드 값이 13일때만 전송 함수를 호출한다.
