...
public class JDBCTemplateDAO {
	...
	/*
	servlet-context.xml에서 생성한 빈을 자동 주입받는다.
	오라클 연결 정보를 가지고 있다.
	 */
	@Autowired
	...
	public void close() {
		/*
		JDBCTemplate에서는 자원해제를 하지 않는다. 그 이유는
	...
	public int write(final SpringBbsDTO springBbsDTO) 
	{
		...
		final로 선언하지 않으면 에러가 발생한다. 
		이것은 Sprind이 아닌 Java의 규칙이다.
		*/
		int result = template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) 
				...
				/*
				하나의 쿼리문 내에서 nextval을 여러번 사용하더라도
				항상 같은 시퀀스를 반환한다.
				 */
	...
	public SpringBbsDTO view(String idx) 
	{
		...
		try {
			/*
			queryForObject() 메서드는 쿼리문을 실행한 후 
			반드시 하나의 결과를 반환해야 한다.
			그렇지 않으면 에러가 발생하게 되므로 예외처리를 하는것이 좋다.
			 */
			...
			이때 테이블의 컬럼명과 DTO의 멤버변수명은 일치해야 한다.
			 */
		}
		catch(Exception e) {
	...
	public int password(String idx, String pass) 
	{
		...
		try {
			...
			*/
			...
			System.out.println("password()실행시 예외발생");
		}
		return retNum;
	}
