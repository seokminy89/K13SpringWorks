...
public class TicketDAO {
	...
	멤버변수 선언
		...
		트랜잭션 처리를 위한 매니져클래스의 멤버변수와 setter()정의
	 */
	...
	public void buyTicket(final TicketDTO dto) {
		...
		try 
		{
			...
			template.update(new PreparedStatementCreator() {
				...
				{
					/*
					티켓의 각겨은 장당 10000원이라 가정하고, 구매수에 곱해서 입력한다.
					 */
			...
			System.out.println("카드 결제와 티켓구매 모두 정상처리 되었습니다.");
			transactionManager.commit(status);
		}
		catch(Exception e) {
			//매수를 초과하면 모든 작업은 rollback처리된다.
