...
/*
기본패키지로 설정한 곳에 컨트롤러를 선언하면 요청이 들어왔을대
...
public class BbsController {
	...
	*/
	...
	public String list(Model model, HttpServletRequest req) {
		...
		이를 ListCommand객체로 전달하기 위해 Model객체에 저장한 후
		매개변수로 전달한다.
		 */
		model.addAttribute("req", req);//request 객체자체를 Model에 저장
	...
	public String view(Model model, HttpServletRequest req) {
		//사용자의 요청을 저장한 request 객체를 Model객체에 저장한 후 전달한다.
		model.addAttribute("req", req);
	...
	public String passwordAction(Model model, HttpServletRequest req) 
	{
		...
		//일련번호는 시퀀스를 사용하기 때문에 1이하의 숫자라면 검증실패로 볼 수 있음.
		...
		if(rowExist<=0) {
			...
			if(mode.equals("edit"))
			{
				...
				/*삭제 후에는 리스트 페이지로 이동(redirect)한다.
				그때 model에 nowPage나 idx같은 파라미터를 입력해주면 
				자동으로 쿼리스트링으로 뒤에 붙여줌.
				*/
			...
			Model model, SpringBbsDTO springBbsDTO) 
	{
		...
		/*
		request내장객체와 수정페이지에서 전송한 모든 폼값을 저장한 
		...
		/*
		수정처리가 완료되면 상세페이지로 이동하게 되는데 이때
		idx와 같은 파라미터가 필요하다.(위에 작성한 writeAction()참조)
		Model객체에 저장한 후 redirect하면 자동으로 쿼리스트링 형태로
		만들어준다.
		 */
		model.addAttribute("idx", req.getParameter("idx"));
		model.addAttribute("nowPage", req.getParameter("nowPage"));
