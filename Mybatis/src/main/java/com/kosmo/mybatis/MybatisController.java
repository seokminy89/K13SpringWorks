...
public class MybatisController {
	...
	public String list(Model model, HttpServletRequest req)
	{
		//방명록 테이블의 게시물 갯수 카운트
		/*
		컨트롤러에서 Service객체 역할을 하는 interface에 정의된
		추상메서드를 호출한다. 
		...
		방명록URL			   -> null로 판단
		첫번째와 두번째 둘 다 값이 없지만 null과 빈값은 다름.
		똑같이 생각했다가는 오류가 날 수 있으니 주의 하기 바람.
		 */
		...
		전달도니 파라미터는 #{param1}과 같이 순서대로 사용한다.
		 */
	...
	{	
		/*
		매핑된 메서드 내에서 session내장 객체를 사용하기 위해
		매개변수로 선언해준다.
		 */
		...
		{	
			/*
			현재 상태는 글쓰기를 위해 버튼을 클릭했으므로,
			만약 로그인이 완료된다면 글쓰기 페이지로 이동하는것이 좋다.
	...
	{	
		/*
		글쓰기 페이지에 오랫동안 머물러 세션이 끊어지는 경우도 있으므로
